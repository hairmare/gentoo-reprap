...
SRC_URI="http://replicatorg.googlecode.com/files/replicatorg-${PV}-linux.tgz"
...
src_install() {
	...
	/opt/replicatorg/machines /opt/replicatorg/tools
	...
	"${D}/opt/replicatorg/bin/replicatorg"
	sed --in-place\
	's/^LD_LIBRARY_PATH.*/LD_LIBRARY_PATH=\/usr\/lib\/rxtx-2\/:\/usr\/lib\/sun-java3d-bin\//'\
	"${D}/opt/replicatorg/bin/replicatorg"
	mv "${D}/usr/share/doc/replicatorg-bin-${PV}" "${D}/opt/replicatorg/doc"
	mv machines/*.xml "${D}/opt/replicatorg/machines/" || die "no machines.xml installed"
